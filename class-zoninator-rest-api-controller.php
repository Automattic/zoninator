...
class Zoninator_Rest_Api_Controller {
    ...
    function __construct( $zoninator ) {
    ...
    function add_post_to_zone($request ) {
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $content = $this->_zoninator->get_admin_zone_post( $post, $zone );
            $status = 200;
        }
    ...
    function zone_update_lock( $request )
    {
        ...
        if (!$this->_zoninator->is_zone_locked($zone_id)) {
            $this->_zoninator->lock_zone($zone_id);
            return new WP_REST_Response(array(
                'zone_id' => $zone_id,
                'status' => 1), 200);
        }
    ...
    function search_posts( $request )
    {
        ...
        $post_types = $this->_zoninator->get_supported_post_types();
        $limit = $this->_get_param( $request, 'limit', $this->_zoninator->posts_per_page );

        if ( 0 >= $limit ) {
            $limit = $this->_zoninator->posts_per_page;
        }
        ...
        if ( $this->_zoninator->_validate_category_filter( $filter_cat ) ) {
            $args['cat'] = $filter_cat;
        }

        if ( $this->_zoninator->_validate_date_filter( $filter_date ) ) {
    ...
    public function get_recent_posts( $request )
    {
        ...
        $limit = $this->_zoninator->posts_per_page;
        $post_types = $this->_zoninator->get_supported_post_types();
        $zone_posts = $this->_zoninator->get_zone_posts( $zone_id );
        ...
        if ( is_wp_error( $zone_posts ) ) {
            ...
            if ( $this->_zoninator->_validate_category_filter( $cat ) ) {
                $args['cat'] = $cat;
            }

            if ( $this->_zoninator->_validate_date_filter( $date ) ) {
